...
namespace OW {
	...
	inline float CalculateTrackBack(Vector3 OwnPos, Vector3 rayOrigin, Vector3 LocalAngle, Vector3 rayDirection) {
		Vector3 intersection;
		float dx, dy, dz, dist, dv, dt;
		Vector3 planeNormal(rayDirection.X, rayDirection.Y, rayDirection.Z);
		__try {
			rayDirection.X = -rayDirection.X;
			rayDirection.Y = -rayDirection.Y;
			rayDirection.Z = -rayDirection.Z;
			float denom = rayDirection.X * planeNormal.X + rayDirection.Y * planeNormal.Y + rayDirection.Z * planeNormal.Z;
			if (std::abs(denom) > 1e-6) { // 避免除以零
				float t = ((OwnPos.X - rayOrigin.X) * planeNormal.X +
					(OwnPos.Y - rayOrigin.Y) * planeNormal.Y +
					(OwnPos.Z - rayOrigin.Z) * planeNormal.Z) / denom;
				intersection = rayOrigin + rayDirection * t;
				if (t > Config::backtracksecond) t = Config::backtracksecond;
				else if (t < -Config::backtracksecond) t = -Config::backtracksecond;
				return t;
			}
			else return 0;
		}
	...
	Vector3 GetVector3(bool predit = false)
	{
		...
		if (entities.size() > 0) {
			{
				if (Config::hanzoautospeed) {
					if (local_entity.HeroID == eHero::HERO_HANJO) {
						Config::predit_level = readult(local_entity.SkillBase, 0xC, 0x2A5) * 85.f + 25.f;
		...
		if (Config::trackback) {
			Config::tracktime = CalculateTrackBack(Vector3(viewMatrix_xor.get_location().x, viewMatrix_xor.get_location().y, viewMatrix_xor.get_location().z), target, SDK->RPM<Vector3>(SDK->g_player_controller + 0x12A0), entities[TarGetIndex].velocity);
		}
		//else
		/* {
